...
export const trackJoinedOnlyButton = () => {
  amplitudeService.customTrack('[Click] 마일리지 참여항목 조회 버튼');
};

export const trackNotJoinedButton = () => {
  amplitudeService.customTrack('[Click] 마일리지 참여항목 조회 버튼');
};
...
export const trackSaveChecklist = () => {
  amplitudeService.customTrack(
    `[Click] 체크리스트 저장 버튼 (서머리 모달 이동)`,
  );
};
