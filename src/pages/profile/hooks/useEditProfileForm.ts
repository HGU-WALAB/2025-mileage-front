...
export const useEditProfileForm = ({ profile, toggleModal }: Props) => {
  const { value: job, handleChange: handleJob } = useInput(profile?.job ?? '');
  const { value: selfDescription, handleChange: handleSelfDescription } = useInput(
    profile?.self_description ?? ''
  );
  const { value: githubLink, handleChange: handleGithubLink } = useInput(
    profile?.github_link ?? ''
  );
  const { value: blogLink, handleChange: handleBlogLink } = useInput(
    profile?.blog_link ?? ''
  );
